...
import math
import torch
from models.feature_exchange import SpatialExchange, ChannelExchange
from models.cross_attention_only_bifea_not_adjacency import CA_BiFeature
...
class SwinTransDecoder(nn.Module):
    ...
    def forward_up_features(self, ca1, ca2, ca3, ca4, x_downsample1, x_downsample2):  # 1/4,1/8,1/16,1/32,     1/32
        x_upsample = []
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = self.transpose_verse(ca1)
                ...
                x = self.transpose(x)
                if inx == 1:
                    x = torch.cat([ca2, ca2], dim=1) + x
                    x = self.conv[inx](x)
                if inx == 2:
                    x = torch.cat([ca3, ca3], dim=1) + x
                    x = self.conv[inx](x)
                if inx == 3:
                    x = torch.cat([ca4, ca4], dim=1) + x
                    x = self.conv[inx](x)
                x = self.transpose_verse(x)  # B,HW,C
                x = self.concat_back_dim[inx](x)  ######
                y1 = layer_up(x)  # layer up 初始层有norm,up  norm,norm,up
    ...
    def forward(self, o1, o2, o3, o4, x_down1, x_down2):
        x, x_upsample = self.forward_up_features(o1, o2, o3, o4, x_down1, x_down2)
...
class encoder1(nn.Module):
    ...
    def forward(self, img1, img2):
        x, y1, y2, x_diff = self.encoder1(img1, img2)
        return x, y1, y2, x_diff
...
class Encoder(nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        x_p, x_2, x_3, x_4 = self.decoder(ca1, ca2, ca3, ca4, x_downsample1, x_downsample2)
